...
async function generateCoinbaseAndDoubleSpend() {
  const privateKey2 = ed.utils.randomPrivateKey();
  const publicKey2 = Buffer.from(await ed.getPublicKey(privateKey2)).toString('hex');
  const coinbase2 = {type: "transaction", height:0, outputs: [{pubkey:publicKey2,value: 50000000000000}]}
...
async function generateBlocks() {
  ...
  blockTestDB = ""
  const [coinbase2, tx1Signed, tx2Signed] = await generateCoinbaseAndDoubleSpend()
  const coinbase2Id = objectToId(coinbase2)
  ...
  blockTestDB = ""
  const [coinbase3, tx3Signed, tx4Signed] = await generateCoinbaseAndDoubleSpend()
  ...
  blockTestDB = ""
  const [coinbase4, tx5Signed, tx6Signed] = await generateCoinbaseAndDoubleSpend()
  ...
  // Mine valid block
  blockTestDB = ""
  const [coinbase5, tx7Signed, tx8Signed] = await generateCoinbaseAndDoubleSpend()
  ...
  const [coinbase6, tx9Signed, tx10Signed] = await generateCoinbaseAndDoubleSpend()
